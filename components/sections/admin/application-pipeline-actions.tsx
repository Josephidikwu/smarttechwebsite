...
/**
 * Generic status-change + notes panel shared by every application pipeline
 * (contact, training, internship, job, general). Takes the two server
 * actions as props so each caller supplies its own bound update/note
 * functions without duplicating this UI a fourth time.
 */
export function ApplicationPipelineActions<Status extends string>({
  id,
  currentStatus,
  statusOptions,
  onUpdateStatus,
  ...
  onUpdateStatus: (id: number, status: Status, note: string) => Promise<void>;
  ...
  function submitStatus() {
    startTransition(async () => {
      await onUpdateStatus(id, status, statusNote);
      setStatusNote("");
      router.refresh();
    });
  }
