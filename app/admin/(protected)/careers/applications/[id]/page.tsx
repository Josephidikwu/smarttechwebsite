...
import { jobApplications, jobs, statusHistory, users, type JobAppStatus } from "@/lib/db/schema";
...
export default async function AdminJobApplicationDetailPage({
  ...
  const [application] = await db
    .select({
      ...
      portfolio: jobApplications.portfolio,
      linkedin: jobApplications.linkedin,
      additionalInfo: jobApplications.additionalInfo,
      cvKey: jobApplications.cvKey,
      coverLetterKey: jobApplications.coverLetterKey,
  ...
  const history = await db
    ...
    .where(and(eq(statusHistory.entityType, "job_application"), eq(statusHistory.entityId, applicationId)))
      ...
      <div className="flex items-start justify-between gap-4">
        <div>
          ...
          <p className="mt-1 text-sm text-[var(--color-ink-muted)]">
            {application.email} · Applying for {application.jobTitle}
          </p>
        </div>
      ...
      <div className="mt-8 grid gap-10 lg:grid-cols-12">
        ...
        <div className="lg:col-span-5">
          <ApplicationPipelineActions
            ...
            onAddNote={addJobApplicationNote}
          />
        </div>
      </div>
    </div>
